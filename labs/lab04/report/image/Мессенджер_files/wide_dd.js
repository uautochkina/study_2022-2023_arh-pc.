...
var wdd = {
    ...
    switch (e.type) {
      ...
      case 'paste':
        ...
        if (e.keyCode == KEY.UP) {
          ...
          if (dd.opts.onSearch) {
            clearTimeout(dd.searchTimer);
            dd.searchTimer = setTimeout(function() {
              dd.opts.onSearch(trim(val(dd.text)), dd);
            }, 0);
          }
        }
      break;
  ...
  _renderList: function(dd, data, showEmpty, append) {
    ...
    var itemsCount = dd.opts.renderLimit !== undefined ? Math.min(dd.opts.renderLimit, data.length) : data.length;
    for (var i = 0, l = itemsCount; i < l; ++i) {
      ...
      var note = q && hl(item[10] || '', q) || (item[10] || '');
      html.push('\
<div class="' + cl + '" onmousedown="WideDropdown.over(\'' + dd.id + '\', \'' + clean(id) + '\'); WideDropdown.select(\'' + dd.id + '\', event)" onmousemove="WideDropdown.over(\'' + dd.id + '\', \'' + clean(id) + '\')" id="wddi' + id + '_' + dd.id + '" onclick="">\
  <div class="wddi_data">' + img + '\
    ...
    <div class="wddi_sub">' + sub + note + '</div>\
  </div>\
</div>');
      ++shown;
    }
    ...
    if (!dd.outdated) {
      dd.list.style.height = (shown > 5) ? (dd.opts.maxListHeight || '242px') : '';
      wdd._showList(dd);
